	...
	switch ($act){
		...
		if($log_row){ 
			if($log_pass==$log_row[2]){
				...
				echo_status(array("respondCode"=>"0","respondMsg"=>"登陆成功","id"=>$log_row['id'],"title"=>$log_row['username'],"nickname"=>$log_row['nickname'],"avatar"=>$log_row['avatar'],"gold"=>$log_row['gold'],"integral"=>$log_row['integral'],"attention"=>$log_row['attention'],"phone"=>$log_row['phone'],"QQ"=>$log_row['QQ'],"email"=>$log_row['email'],"regtime"=>$log_row['regtime'],"rank"=>$log_row['rank']));
			}else{
				
				echo_status(array('respondMsg'=>'用户名或者密码错误!','respondCode'=>'1'));		
			}
		}else{
			echo_status(array('respondMsg'=>'对不起,没有此用户!','respondCode'=>'1'));	
		};
		break;
