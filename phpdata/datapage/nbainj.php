...
switch($type){
	...
	case 'GetInjuriesList':
		...
		while ($row=mysql_fetch_array($query)) {
			...
			$result[$i] = array('id' => $row["id"] , "teamid" =>$row["team"] , "teamName" =>$teamName["team"] , "player" =>$row["player"] , "state" =>$row["state"] , "reason" =>$row["reason"] , "timetable" =>$row["timetable"] , "datetime" =>$row["datetime"] );
			$i++;
